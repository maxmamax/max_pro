...
$sql1 = mysql_query("SELECT student_name,student_last FROM tb_student WHERE student_key = '".$_SESSION['session_key']."' ");
$fet1 = mysql_fetch_array($sql1);
?>
<div  style='text-align:center'>
  <h1>บันทึกการฝึกงานประจำวันของนักศึกษา</h1> ( <?php echo $fet1['student_name']." ".$fet1['student_last']; ?>)
  <h3>( <?php echo $fet['position_name']; ?> )</h1>
</div>
<div class="" >
<?php
if($_POST['student_key'] != ""){
  $id = $_POST['student_key'];
}
if($_SESSION['session_key'] != ""){
  $id = $_SESSION['session_key'];
}
$sql = mysql_query("SELECT * FROM tb_record WHERE position_keyf = '".$_POST['id']."' AND student_keyf = '$id' ");
?>
  <table class="table" >
    <tr>
