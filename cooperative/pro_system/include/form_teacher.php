...
$sql = mysql_query("SELECT count(teacher_user) as a FROM tb_teacher WHERE teacher_user = '".'T_'.$_POST['username']."' ");
$fet = mysql_fetch_array($sql);
if($fet['a'] == '0'){
$a = strrchr($_FILES['file']['name'],".");
$part = "T_".$_POST['username'].$a;
$sql = mysql_query("INSERT INTO tb_teacher VALUES  ('','".'T_'.$_POST['username']."','".$_POST['password']."','".$_POST['name']."','".$_POST['lname']."','".$_POST['code']."','".$_POST['email']."','".$_POST['r']."','$part','".$_POST['select_faculty']."','1') ");
$part = iconv("UTF-8", "TIS-620", "T_".$_POST['username']);
copy($_FILES['file']['tmp_name'],'include/image_teacher/'.$part.$a );
$sql = mysql_query("SELECT * FROM tb_teacher WHERE teacher_user = '".'T_'.$_POST['username']."' ");
$fet = mysql_fetch_array($sql);
$rand = 'T_'.substr(str_shuffle('ABCDEFGHIJKLMNOPQRSTUVWXYZ23456789'),0,5);
$sql2 = mysql_query("INSERT INTO tb_security_teacher VALUES ('".$fet['teacher_key']."','$rand') ") ;
$i = sentEmail('T_'.$_POST['username'],$_POST['password'],$rand,$_POST['email']);
if($i == 'Message sent!'){ ?>
...
<select class="form-control" name="r" id="r">
  <option value="">-- ประเภทอาจารย์ --</option>
  <option value="T">อาจารย์ที่ปรึกษา</option>
  <option value="S">อาจารย์ประจำหลักสูตร</option>
  <option value="TS">อาจารย์ประจำหลักสูตรและอาจารย์ประจำหลักสูตร</option>
</select>
  <span id="r_a"></span>
</div>
</div>
  ...
  <div class="form-group row">
  <div class=" col-md-3">
    <label for="ex1"><font class="f1">อัพโหลดไฟล์รูป <font style="color:red;font-size:25px;"> * </font> </font> <span  class="glyphicon glyphicon-question-sign btn btn-warning" aria-hidden="true" data-toggle="tooltip" title="*หมายเหตุ! อัปโหลดรูปภาพประจำตัวนามสกุลไฟล์ jpg,png เท่านั้น"></span></label>
...
$("#upload_a").html("<font style='color:red'><img src='img/validate.gif' width='20' heigth='20'>กรุณากรอกเลือกไฟล์</font>");
  vuploade = 0;
}
...
if( a != "" && vuser == 1 && vpassword == 1 && vemail == 1 && vuploade == 1 && name != ''  && lname  != '' && code  != '' && email != '' && select_faculty != '' && r != ''&& upload != ''  ){
  var txt;
