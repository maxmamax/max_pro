...
$sql = mysql_query("SELECT count(staff_user) as a FROM tb_staff WHERE staff_user = '".'P_'.$_POST['username']."' ");
$fet = mysql_fetch_array($sql);
if($fet['a'] == '0'){
$a = strrchr($_FILES['file']['name'],".");
$part = "P_".$_POST['username'].$a;
$sql = mysql_query("INSERT INTO tb_staff VALUES  ('','".'P_'.$_POST['username']."','".$_POST['password']."','".$_POST['name']."','".$_POST['lname']."','".$_POST['code']."','".$_POST['email']."','$part','".$_POST['select_faculty']."','1') ");
$part = iconv("UTF-8", "TIS-620", "P_".$_POST['username']);
copy($_FILES['file']['tmp_name'],'include/image_staff/'.$part.$a);
$sql = mysql_query("SELECT * FROM tb_staff WHERE staff_user = '".'P_'.$_POST['username']."' ");
$fet = mysql_fetch_array($sql);
$rand = 'P_'.substr(str_shuffle('ABCDEFGHIJKLMNOPQRSTUVWXYZ23456789'),0,5);
$sql2 = mysql_query("INSERT INTO tb_security_staff VALUES ('".$fet['staff_key']."','$rand') ") ;
$i = sentEmail('P_'.$_POST['username'],$_POST['password'],$rand,$_POST['email']);
if($i == 'Message sent!'){ ?>
...
<div class=" col-md-3">
  ...
  <div class="form-group row">
  <div class=" col-md-3">
    <label for="ex1"><font class="f1">รหัสพนักงาน <font style="color:red;font-size:25px;"> * </font></font></label>
    <input class="form-control" id="code" type="number" placeholder="รหัสพนักงาน" name="code">
    <span id="code_a"></span>
  </div>
  <div class="form-group row">
  <div class=" col-md-3">
